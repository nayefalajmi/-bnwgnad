...
export default async function handler(req) {
  ...
  /* مبيعات المتاجر (محسوبة في الفرونت من ملفات CSV) — نمرّرها كما هي في الـ meta.
     تشمل: current (كامل الفترة)، stores (تفصيل كل متجر)، rangeLabel، storeNames. */
  const shopMeta = needShop && shop ? shop : null;

  const sourceLabel = dataSource === 'both' ? 'الموقع + المتاجر (مدموجان)'
                    : dataSource === 'shop' ? 'المتاجر فقط'
                    : 'الموقع فقط';
  ...
  const system =
    ...
    '- العملة دائماً الدينار الكويتي (د.ك)، والدينار الواحد = 1000 فلس (وليس 100 فلس).\n' +
    '- لا تستخدم كلمة "فلساً/فلس" إلا إذا كان المبلغ أقل من دينار واحد؛ المبالغ بالدنانير تُذكر بالدينار (د.ك).\n' +
    '- للأرقام الكبيرة استخدم فاصلة الآلاف لتسهيل القراءة (مثل: 48,812 د.ك).\n' +
    '- عند ذكر أي نسبة مئوية، تحقّق من صحة حسابها من الأرقام المعطاة قبل كتابتها، ولا تذكر نسبة غير متّسقة مع البيانات.\n' +
    '- اجعل كل توصية مبنية على الأرقام الفعلية المعطاة فقط، دون أي تخمين أو افتراض.\n' +
    '- إذا وجدت تناقضاً أو رقماً غير منطقي في البيانات، نبّه عليه صراحةً بدل تجاهله.\n' +
    ...
    '    • "المحل/المتاجر" = مبيعات الفروع من ملفات المحاسبة (الحقل shop: إجمالي البيع، المستردات، الخصومات، صافي المبيعات، تكلفة البضاعة، إجمالي الربح، الهامش). بيانات المتاجر تغطّي كامل الفترة الموجودة في الملف (shop.rangeLabel) بغضّ النظر عن فترة الموقع، وقد تكون مدموجة من عدة متاجر (shop.stores، shop.storeNames).\n' +
    '- مهم جداً: عند ذكر أي رقم، وضّح مصدره صراحةً بكلمة بين قوسين: (موقع) أو (محل) أو (إجمالي).\n' +
    '- إذا تعددت المتاجر (shop.stores)، يمكنك ذكر أبرز فروقاتها باختصار.\n' +
